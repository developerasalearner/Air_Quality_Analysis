...
def main():
    ...
    # Option to detect location using GPS
    detect_location = st.checkbox('Auto Detect Location')
    ...
    if detect_location:
        ...
        if st.button('Get Current Location'):
            try:
                user_location = locator.geocode("")
                if user_location is not None:
                    st.write(f'Your Current Location: {user_location.address}')
                    location = user_location.address
                else:
                    st.write("Location could not be detected. Please enter manually.")
